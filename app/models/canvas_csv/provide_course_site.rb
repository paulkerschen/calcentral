  ...
  class ProvideCourseSite < Base
    ...
    def retrieve_course_site_details
      ...
      if response[:statusCode] == 200 && (course_properties = response[:body]) && (canvas_course_id = course_properties['id'])
        settings_worker = Canvas::CourseSettings.new(course_id: canvas_course_id)
        settings_worker.fix_default_view course_properties
        settings_worker.hide_grade_distributions
        
        tabs_worker = Canvas::ExternalTools.new(canvas_course_id: canvas_course_id)
