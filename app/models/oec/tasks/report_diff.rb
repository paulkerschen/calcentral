module Oec
  module Tasks
    class ReportDiff < Base
      ...
      COLUMNS_TO_COMPARE = %w(COURSE_NAME FIRST_NAME LAST_NAME EMAIL_ADDRESS DEPT_FORM EVALUATION_TYPE START_DATE END_DATE)
      ...
      def diff_for_department(dept_code)
        diff_report = Oec::Worksheets::DiffReport.new @opts
        dept_name = Berkeley::Departments.get(dept_code, concise: true)
        validate(dept_code, @term_code) do |errors|
          unless (sis_data = csv_row_hash([@term_code, Oec::Folder.sis_imports, "#{datestamp} #{timestamp}", dept_name], dept_code, Oec::Worksheets::SisImport))
            log :warn, "Skipping #{dept_name} diff: no '#{Oec::Folder.sis_imports}' '#{datestamp} #{timestamp}' spreadsheet found"
            return
          end
          unless (dept_data = csv_row_hash([@term_code, Oec::Folder.confirmations, dept_name], dept_code, Oec::Worksheets::CourseConfirmation))
      ...
      def csv_row_hash(folder_names, dept_code, klass)
        ...
        klass.from_csv(csv, dept_code: dept_code, term_code: @term_code).each do |row|
          begin
            row = Oec::Worksheets::Base.capitalize_keys row
            if (id = extract_id row)
              validate(dept_code, id[:ccn]) do |errors|
                report(errors, id, :annotation, false, %w(A B GSI CHEM MCB MID))
