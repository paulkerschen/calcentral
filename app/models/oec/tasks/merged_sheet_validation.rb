  ...
  module Tasks
    module MergedSheetValidation
      ...
      def build_and_validate_export_sheets
        ...
        course_confirmations = Oec::Worksheets::SisImport.from_csv(@remote_drive.export_csv(course_confirmations_file), dept_code: nil, term_code: @term_code)
        ...
        supervisor_overrides = Oec::Worksheets::Supervisors.from_csv @remote_drive.export_csv(supervisors_sheet)

        instructors = Oec::Worksheets::Instructors.new
        course_instructors = Oec::Worksheets::CourseInstructors.new
        courses = Oec::Worksheets::Courses.new
        students = Oec::Worksheets::Students.new
        course_students = Oec::Worksheets::CourseStudents.new
        supervisors = Oec::Worksheets::Supervisors.new
        department_hierarchy = Oec::Worksheets::DepartmentHierarchy.new
        report_viewer_hierarchy = Oec::Worksheets::ReportViewerHierarchy.new

        if (previous_course_supervisors = @remote_drive.find_nested [@term_code, Oec::Folder.overrides, 'course_supervisors'])
          course_supervisors = Oec::Worksheets::CourseSupervisors.from_csv @remote_drive.export_csv(previous_course_supervisors)
        else
          course_supervisors = Oec::Worksheets::CourseSupervisors.new
        end
        ...
        if (previous_term_folder = find_previous_term_folder) && (previous_term_last_export = find_last_export previous_term_folder)
          previous_course_instructors = worksheet_from_folder(previous_term_last_export, Oec::Worksheets::CourseInstructors)
          previous_instructors = worksheet_from_folder(previous_term_last_export, Oec::Worksheets::Instructors)
        ...
        participating_dept_forms = participating_dept_names.collect {|d| Oec::Worksheets::Base.dept_form_from_name d}
        ...
        course_confirmations.each do |confirmation|
          ...
          validate('courses', confirmation['COURSE_ID']) do |errors|
            ...
            if default_dates
              ...
              errors.add "START_DATE #{confirmation['START_DATE'] || 'nil'} does not match term code #{@term_code}" unless confirmation['START_DATE']&.end_with? @term_code[0..3]
              errors.add "END_DATE #{confirmation['END_DATE'] || 'nil'} does not match term code #{@term_code}" unless confirmation['END_DATE']&.end_with? @term_code[0..3]
            end
