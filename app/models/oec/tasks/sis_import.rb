module Oec
  module Tasks
    class SisImport < Base
      ...
      on_success_run Oec::Tasks::ReportDiff, if: proc { !@opts[:local_write] }
      ...
      def run_internal
        ...
        Oec::DepartmentMappings.new(term_code: @term_code).by_dept_code(@departments_filter).each do |dept_code, course_codes|
          ...
          worksheet = Oec::Worksheets::SisImport.new(dept_code: dept_code, term_code: @term_code)
      ...
      def import_courses(worksheet, course_codes)
        ...
        apply_overrides(worksheet, Oec::Worksheets::Courses, course_codes, %w(DEPT_NAME CATALOG_ID INSTRUCTION_FORMAT SECTION_NUM))
        apply_overrides(worksheet, Oec::Worksheets::Instructors, course_codes, %w(LDAP_UID SIS_ID))
      ...
      def import_course(worksheet, course)
        ...
        unless worksheet[row_key]
          if course['enrollment_count'].to_i.zero?
            ...
            worksheet[row_key] = Oec::Worksheets::Base.capitalize_keys course
          end
        end
      end
      ...
      def set_dept_form(worksheet, course)
        ...
        # regardless of subject area.
          ...
          course['dept_form'] = Oec::Worksheets::Base.dept_form_from_name course['dept_name']
        end
      end
      ...
      def apply_overrides(worksheet, type, course_codes, select_columns)
        ...
        overrides_sheet.each do |overrides_row|
          next if (type == Oec::Worksheets::Courses) && !course_codes.find do |code|
        ...
        if (type == Oec::Worksheets::Courses) && @term_dates
