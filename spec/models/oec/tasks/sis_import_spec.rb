describe Oec::Tasks::SisImport do
  let(:term_code) { '2015-B' }
  let(:task) { described_class.new(term_code: term_code, local_write: local_write, allow_past_term: true) }

  let(:fake_remote_drive) { double() }
  let(:course_overrides_row) { Oec::Worksheets::Courses.new.headers.join(',') }
  let(:instructor_overrides_row) { Oec::Worksheets::Instructors.new.headers.join(',') }
  ...
  before(:each) do
    ...
    allow(fake_remote_drive).to receive(:find_nested).with([term_code, Oec::Folder.overrides, Oec::Worksheets::Courses.export_name]).and_return course_overrides
    ...
    allow(fake_remote_drive).to receive(:find_nested).with([term_code, Oec::Folder.overrides, Oec::Worksheets::Instructors.export_name]).and_return instructor_overrides
  ...
  shared_context 'local-write mode and no follow-up diff' do
    ...
    before do
      expect(Oec::Tasks::ReportDiff).not_to receive(:new)
    end
  end
  ...
  shared_context 'follow-up diff and no local-write mode' do
    ...
    before do
      expect(Oec::Tasks::ReportDiff).to receive(:new).and_call_original
      expect_any_instance_of(Oec::Tasks::ReportDiff).to receive(:date_time_of_most_recent).and_return DateTime.now
  ...
  describe 'CSV export' do
    subject do
      ...
      allow(Oec::Worksheets::SisImport).to receive(:new).and_return courses
      task.run_internal
      courses.write_csv
    ...
    let(:courses) { Oec::Worksheets::SisImport.new(dept_code: l4_codes[dept_name]) }
    let(:courses_by_ccn) { {} }
      ...
      it 'should include DEPT_FORM only for non-crosslisted courses' do
        subject.each do |row|
          if row['CROSS_LISTED_FLAG'] == 'Y'
            ...
            expect(row['DEPT_FORM']).to eq Oec::Worksheets::Base.dept_form_from_name row['DEPT_NAME']
          end
        end
      end
    ...
    describe 'expected network operations' do
      subject { Oec::Tasks::SisImport.new(term_code: term_code) }
      ...
      it 'should upload a department csv and a log file' do
        ...
        expect(fake_remote_drive).to receive(:check_conflicts_and_upload)
          .with(kind_of(Oec::Worksheets::Base), sheet_name, (Oec::Worksheets::Base), imports_today_folder, anything)
  ...
  context 'department-specific filters' do
    ...
    it 'filters by department codes' do
      ...
      Oec::Tasks::SisImport.new(default_opts.merge(dept_codes: 'IBIBI IMMCB')).run
    end
    ...
    it 'understands all_participating option' do
      ...
      Oec::Tasks::SisImport.new(default_opts.merge(dept_codes: 'all_participating')).run
    end
  end

  context 'task not told to allow past terms' do
    let(:task) { Oec::Tasks::SisImport.new(term_code: term_code, local_write: true) }
