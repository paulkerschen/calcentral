describe CanvasCourseGradeExportController do
  ...
  describe 'when preparing course enrollments cache' do
    ...
    it_should_behave_like 'an endpoint' do
      let(:make_request) { post :prepare_grades_cache, params: {canvas_course_id: canvas_course_id, format: :csv} }
    ...
    it 'makes call to load canvas course student grades with forced cacheing' do
      ...
      allow_any_instance_of(CanvasLti::Egrades).to receive(:background).and_return(torquebox_fake_background_proxy)
      post :prepare_grades_cache, params: {canvas_course_id: canvas_course_id, format: :csv}
  ...
  describe '#job_status' do
    ...
    it_should_behave_like 'an endpoint' do
      let(:error_text) { 'Something went wrong' }
      let(:make_request) { get :job_status, params: {canvas_course_id: canvas_course_id, jobId: background_job_id} }
    ...
    it 'returns error if egrades background job not found' do
      get :job_status, params: {canvas_course_id: canvas_course_id, jobId: background_job_id}
    ...
    it 'returns status of canvas egrades background job' do
      ...
      get :job_status, params: {canvas_course_id: canvas_course_id, jobId: egrades.background_job_id}
      assert_response :success
      json_response = JSON.parse(response.body)
  ...
  describe 'when serving grade export option data' do
    ...
    it_should_behave_like 'an endpoint' do
      let(:make_request) { get :export_options, params: {canvas_course_id: 'embedded', format: :csv} }
      let(:error_text) { 'Something went wrong' }
    ...
    it 'provides export options' do
      get :export_options, params: {canvas_course_id: 'embedded'}
      expect(response.status).to eq(200)
      json_response = JSON.parse(response.body)
      expect(json_response).to be_an_instance_of Hash
    ...
    it 'supports canvas course id parameter when absent in session' do
      ...
      get :export_options, params: {canvas_course_id: canvas_course_id}
      expect(response.status).to eq(200)
      json_response = JSON.parse(response.body)
      expect(json_response).to be_an_instance_of Hash
      official_sections = json_response['officialSections']
  ...
  describe 'when serving egrades download' do
    ...
    it_should_behave_like 'an endpoint' do
      let(:make_request) { get :download_egrades_csv, params: {canvas_course_id: 'embedded', format: :csv, term_cd: 'D', term_yr: '2014', ccn: '1234', type: 'final', pnp_cutoff: 'C-'} }
      let(:error_text) { 'Something went wrong' }
      before { allow_any_instance_of(CanvasLti::Egrades).to receive(:official_student_grades_csv).and_raise(RuntimeError, error_text) }
    end

    it_should_behave_like 'an authenticated endpoint' do
      let(:make_request) { get :download_egrades_csv, params: {canvas_course_id: 'embedded', format: :csv, term_cd: 'D', term_yr: '2014', ccn: '1234'} }
    end

    context 'when the canvas course id is not present in the session' do
      before { session['canvas_course_id'] = nil }
      it 'returns 403 error' do
        get :download_egrades_csv, params: {canvas_course_id: 'embedded', format: :csv, term_cd: 'D', term_yr: '2014', ccn: '1234'}
    ...
    context 'when user is authorized to download egrades csv' do
      ...
      it 'raises exception if term code not provided' do
        get :download_egrades_csv, params: {canvas_course_id: 'embedded', format: :csv, term_yr: '2014', ccn: '1234', type: 'final', pnp_cutoff: 'C-'}
      ...
      it 'raises exception if term year not provided' do
        get :download_egrades_csv, params: {canvas_course_id: 'embedded', format: :csv, term_cd: 'D', ccn: '1234', type: 'final', pnp_cutoff: 'C-'}
        expect(response.status).to eq(400)
        json_response = JSON.parse(response.body)
      ...
      it 'raises exception if course control number not provided' do
        get :download_egrades_csv, params: {canvas_course_id: 'embedded', format: :csv, term_cd: 'D', term_yr: '2014', type: 'final', pnp_cutoff: 'C-'}
        expect(response.status).to eq(400)
        json_response = JSON.parse(response.body)
      ...
      it 'raises exception if type not provided' do
        get :download_egrades_csv, params: {canvas_course_id: 'embedded', format: :csv, term_cd: 'D', term_yr: '2014', ccn: '1234', pnp_cutoff: 'C-'}
        expect(response.status).to eq(400)
        json_response = JSON.parse(response.body)
      ...
      it 'raises exception if P/NP cutoff not provided' do
        get :download_egrades_csv, params: {canvas_course_id: 'embedded', format: :csv, term_cd: 'D', term_yr: '2014', ccn: '1234', type: 'final'}
        expect(response.status).to eq(400)
        json_response = JSON.parse(response.body)
      ...
      it 'raises exception if P/NP cutoff not parseable' do
        get :download_egrades_csv, params: {canvas_course_id: 'embedded', format: :csv, term_cd: 'D', term_yr: '2014', ccn: '1234', type: 'final', pnp_cutoff: 'XYZ'}
        expect(response.status).to eq(400)
        json_response = JSON.parse(response.body)
      ...
      it 'serves egrades csv file download' do
        get :download_egrades_csv, params: {canvas_course_id: 'embedded', format: :csv, term_cd: 'D', term_yr: '2014', ccn: '1234', type: 'current', pnp_cutoff: 'C-'}
        expect(response.status).to eq(200)
      ...
      it 'supports canvas course id parameter when absent in session' do
        session['canvas_course_id'] = nil
        get :download_egrades_csv, params: {canvas_course_id: canvas_course_id, :format => :csv, :term_cd => 'D', :term_yr => '2014', :ccn => '1234', :type => 'current', pnp_cutoff: 'C-'}
        expect(response.status).to eq(200)
        expect(response.headers['Content-Type']).to eq 'text/csv'
        expect(response.headers['Content-Disposition']).to eq 'attachment; filename=egrades-current-1234-Fall-2014-1164764.csv'
  ...
  describe 'when indicating if a course site has official sections' do
    ...
    it_should_behave_like 'an endpoint' do
      let(:make_request) { get :is_official_course, params: {format: :csv, canvas_course_id: '1234'} }
      let(:error_text) { 'Something went wrong' }
    ...
    context 'when the canvas course id is not present in the params' do
      it 'returns 403 error' do
        get :is_official_course, params: {format: :csv}
        expect(response.status).to eq(403)
        expect(response.body).to eq ''
      end
    end
    ...
    it 'should set cross origin access control headers' do
      get :is_official_course, params: {format: :csv, canvas_course_id: '1234'}
